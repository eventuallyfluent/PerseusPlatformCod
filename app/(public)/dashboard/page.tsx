        ...
        <div className="space-y-7">
          <section className="space-y-2">
            <div className="space-y-2">
              ...
              <h1 className="text-4xl leading-tight text-[var(--portal-text)] sm:text-5xl">Your library</h1>
            </div>
          </section>

          <div className="space-y-5">
            <section className="grid gap-3 md:grid-cols-3">
              <div className="perseus-dashboard-stat rounded-[20px] border border-[var(--portal-border)] bg-[var(--portal-panel-strong)] p-5 text-[var(--portal-text)]">
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--portal-muted)]">Owned courses</p>
                <p className="mt-2 text-3xl font-semibold">{hydratedCourseStates.length}</p>
              </div>
              <div className="perseus-dashboard-stat rounded-[20px] border border-[var(--portal-border)] bg-[var(--portal-panel-strong)] p-5 text-[var(--portal-text)]">
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--portal-muted)]">Lessons completed</p>
                <p className="mt-2 text-3xl font-semibold">{hydratedCourseStates.reduce((sum, item) => sum + item.state.completionCount, 0)}</p>
              </div>
              <div className="perseus-dashboard-stat rounded-[20px] border border-[var(--portal-border)] bg-[var(--portal-panel-strong)] p-5 text-[var(--portal-text)]">
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--portal-muted)]">Ready to continue</p>
                <p className="mt-2 text-3xl font-semibold">{hydratedCourseStates.filter((item) => item.state.nextLesson && item.state.completionCount > 0).length}</p>
              </div>
            </section>
            ...
            <section className="space-y-5">
              <div className="grid gap-4 xl:grid-cols-2">
                {hydratedCourseStates.map(({ enrollment, state }) => (
                  <article key={enrollment.id} className="perseus-dashboard-course-card rounded-[22px] border border-[var(--portal-border)] bg-[var(--portal-panel)] p-5 text-[var(--portal-text)] shadow-[0_18px_34px_rgba(10,11,24,0.18)]">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div className="min-w-0 space-y-2.5">
                        <Badge variant={state.nextLockedLesson ? "warning" : "success"}>{state.statusLabel}</Badge>
                        <h3 className="text-2xl font-semibold leading-tight sm:text-3xl">{enrollment.course.title}</h3>
                        <p className="text-sm leading-6 text-[var(--portal-muted)]">{enrollment.course.instructor.name}</p>
                      </div>
                    ...
                    <div className="mt-5 rounded-[18px] border border-[var(--portal-border)] bg-[var(--surface-panel-strong)] px-4 py-4">
                      {state.nextLesson ? (
              ...
              {courseStates.length === 0 ? (
                <section className="rounded-[22px] border border-[var(--portal-border)] bg-[var(--portal-panel)] p-6 text-[var(--portal-text)]">
                  <h2 className="text-2xl font-semibold leading-tight">No courses yet.</h2>
                </section>
              ) : null}
            </section>
          </div>
